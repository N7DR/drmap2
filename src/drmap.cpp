...
const unsigned int N_CPUS { thread::hardware_concurrency() }; // so we can control the number of threads sensibly

bool            debug   { false };
optional<float> NO_DATA { };    // default no_data value

const string NODATA_COLOUR { "aquamarine4"s };              // colour on plots when data are missing

int    n_cells       { 300 };                                                          // number of cells to be displayed from centre to outside of a plot
...
map<int /* lat-long code */, geotiff_tile> tiles;            // container for the actual tiles we will use
...
void call_lat_long(RInside& R, const string& callsign, const double latitude, const double longitude);                                                                          ///< write the callsign, latitude and longitude information on the plot
...
float elevation_angle(const double lat1, const double long1, const double lat2, const double long2, const double h1, const double h2);
...
*/
inline float elevation_angle(const pair<double, double>& ll1, const pair<double, double>& ll2, const double h1, const double h2)
  { return elevation_angle(ll1.first, ll1.second, ll2.first, ll2.second, h1, h2); }

// return a command line value in metric units
float command_line_value(const command_line& cl, const string_view parameter, const float default_value, const UNITS units)
{ using enum UNITS;

  float rv { static_cast<float>(default_value * ( (units == IMPERIAL) ? FTOM : 1)) };

  if (cl.value_present(parameter) and !starts_with(cl.value(parameter), "-"s))  
    rv = from_string<float>(cl.value(parameter)) * ((units == IMPERIAL) ? FTOM : 1);
...
// read values from the command line 
  const UNITS units    { cl.parameter_present("-imperial"s) ? UNITS::IMPERIAL : UNITS::METRIC };
  ...
  const float  antenna_height  { command_line_value(cl, "-ant"s, 0, units) };                                                                // converted to metres
  const float  los_height      { command_line_value(cl, "-los"s, (antenna_height ? antenna_height * MTOF : ( imperial ? 5 : 1.5)), units) };  // metres; 5 => eye_level = 5 feet
  ...
  const string distance_unit_str      { (imperial ? "mi"sv : "km"sv) };
  const string height_unit_str        { (imperial ? "ft"sv : "m"sv) };
  const string long_distance_unit_str { (imperial ? "miles"sv : "km"sv) };

// check that directories exist
  if (!directory_exists(data_directory, LINKS::INCLUDE))
  { cerr << "data directory does not exist: " << data_directory << endl;
    exit(-1);
  }
...
// try to read lat/long info from QTH file -- only if lat/long not set
  ...
  { if (!file_exists(qth_db_filename))
    ...
    const vector<string> lines { squash(to_lines <string> (to_upper(read_file(qth_db_filename))), ' ') };
    
    double db_latitude  { 0 };      // latitude of call's QTH
    double db_longitude { 0 };      // longitude of call's QTH
    ...
    { const string& line { lines[n] };
      ...
      if (!line.empty())
      { const vector<string_view> fields { split_string <string_view> (line, ' ') };
    ...
    if (found_call)                   // assign correct lat and long from call database
...
// the default plots -- maybe remove these and require explicit values?
  const vector<double> imperial_distances { 1 * MITOKM * 1000, 2 * MITOKM * 1000, 5 * MITOKM * 1000};   // distances are always in metric units
  const vector<double> metric_distances   { 1000, 2000, 5000, 10'000 };
  
  vector<double> cli_distances;         // to hold distances from the command line
  
  if (cl.value_present("-radius"s))
    FOR_ALL(split_string <string_view> (cl.value("-radius"s), ','), [imperial, &cli_distances] (const string_view r) { cli_distances += (from_string<int>(r) * 1000 * (imperial ? MITOKM : 1)); });
  ...
  SORT(distances_m);         // always go from smallest to largest area
...
// to be easily run in multiple threads without having to deal with asynchronous downloads  
    tile_llcs.clear();  
    tile_llcs += llc(qth);                   // we need at least the tile that contains the QTH

    if (debug)
    { cout << "distance per square = " << distance_per_square << " metres" << endl;
...
// download the new tiles in parallel
    { vector<future<void>> vec_futures;    
      ...
      for (const auto& tile_llc : tile_llcs)
        vec_futures.emplace_back(async(launch::async, download_tiff_file_if_necessary, tile_llc, data_directory));

      if (debug)
        cout << "Number of futures in vector: " << vec_futures.size() << endl;

      for (auto& this_future : vec_futures)
        this_future.get();                                  // .get() blocks until the future is available
    }

// at this point, we have all the tiles

// make the tiles available in the tiles vector
    for (auto tile_llc : tile_llcs)
    { tiles[tile_llc] = geotiff_tile { local_tile_filename(tile_llc, data_directory) };

      if (tiles[tile_llc].n_bands() != 1)                                                                         // check that each tile has only a single band
      { cerr << "Tile " << tile_llc << " has incorrect number of bands: " << tiles[tile_llc].n_bands() << endl;
        exit(-1);
      }

      if (NO_DATA)                                                                                                // check that we have consistent NODATA values
      { if (NO_DATA.value() != tiles[tile_llc].no_data())
        { cerr << "Inconsistent NO_DATA values; prior value = " << NO_DATA.value() << "; value from tile = " << tiles[tile_llc].no_data() << endl;
          exit(-1);
        }
      }

      NO_DATA = tiles[tile_llc].no_data();        // set the global value of NO_DATA
    }
    
    if (debug)
      cout << "Calculating map for distance = " << comma_separated_string(int(distance_scale + 0.5)) << endl;

// create fields of correct sizes with default values -- we don't have std::mdspan support yet
    ...
    if (debug)
    { cout << "qth = (" << qth.first << ", " << qth.second << ")" << endl;
      cout << "llc(qth) = " << llc(qth) << endl;
    }
...
// this would be vastly more efficient with mdspan; could use min_element on the entire (linear) array
...
// the basic height map
    create_figure(R, out_directory + "/drmap2-"s + modified_callsign + "-" + distance_str + distance_unit_str + ".png"s, width, ( (3 * width) / 4 ));
...
// create distance axes
    ...
    vector<int> distances_km { (ds < 10'000) ? r_seq( static_cast<int>(-ds / 1000), static_cast<int>(ds / 1000) ) :
                                  ( (ds < 20'000) ? r_seq( static_cast<int>(-ds / 1000), static_cast<int>(ds / 1000), 2 ) :
                                  ( (ds < 50'000) ? r_seq( static_cast<int>(-ds / 1000), static_cast<int>(ds / 1000), 5 ) :
                                  ( (ds < 100'000) ? r_seq( static_cast<int>(-ds / 1000), static_cast<int>(ds / 1000), 10 ) :
 ...
 // calculate the actual locations of the ticks and labels; this is (always) measured in metres
    ...
    vector<int> distances_in_metres { (ds < 10'000) ? r_seq( -rds, rds, static_cast<int>(1000.01 * ( imperial ? MITOKM : 1)) ) :
                                      (ds < 20'000) ? r_seq( -rds, rds, static_cast<int>(2000.01 * ( imperial ? MITOKM : 1)) ) :
                                      (ds < 50'000) ? r_seq( -rds, rds, static_cast<int>(5000.01 * ( imperial ? MITOKM : 1)) ) :
                                      (ds < 100'000) ? r_seq( -rds, rds, static_cast<int>(10000.01 * ( imperial ? MITOKM : 1)) ) :
...
// gradient
    ...
    { if (debug)
      ...
      create_figure(R, out_directory + "/drmap2-"s + modified_callsign + "-" + distance_str + distance_unit_str + "-los.png"s, width, ( (3 * width) / 4 ));
      create_screens(R, screen_definitions);
      select_screen(R, 1);
...
// gradient
    ...
    { if (debug)
      ...
      create_figure(R, out_directory + "/drmap2-"s + modified_callsign + "-" + distance_str + distance_unit_str + "-elev.png"s, width, ( (3 * width) / 4 ));
      create_screens(R, screen_definitions);
      select_screen(R, 1);
...
// use ranked angles instead of absolute values in order to linearise the gradient
      ...
      auto mapped_angle = [&angles](const float& a)
...
// we can't use the usual canned routines to display the gradient because there's no simple value-based mapping
    ...
    { if (debug)
      ...
      create_figure(R, out_directory + "/drmap2-"s + modified_callsign + "-" + distance_str + distance_unit_str + "-grad.png"s, width, ( (3 * width) / 4 ));
      create_screens(R, screen_definitions);
      select_screen(R, 1);
...
/*! \brief                          Determine, in parallel, the needed tiles
    ...
    Calculations relating to hzn are not performed, as those need be done only once, not per-cell
*/
...
/// push back the x or y coordinates of the points [1, 2, 3, 4] into v
  auto pb = [] (vector<float>& v, const float p1, const float p2, const float p3, const float p4) { v += vector { p1, p2, p3, p4, static_cast<float>(NA_REAL) }; };
    
  for (int bearing { 0 }; bearing < 360; ++bearing)
  { clr += cv[vm_horizon.map_value(horizon[bearing])];      // the colour of this quadrilateral

    const auto θ_1 { bearing * DTOR };                      // most anticlockwise angle [1, 2]
    const auto θ_2 { (bearing + 1) * DTOR };                // most clockwise angle [3, 4]
    const auto t1  { tan(θ_1) };
    const auto t2  { tan(θ_2) };
...
{ const double d  { distance(lat1, long1, lat2, long2) };
  const double θ  { d / RE };
  const double OD { (RE + h1) / cos(θ) };
  const double AD { (RE + h1) * tan(θ) };
  const double BD { OD - (RE + h2) };
  const double AB { sqrt(AD * AD + BD * BD - 2 * AD * BD * sin(θ)) };  // cosine rule
  const double α  { -asin( (BD * cos(θ)) / AB ) };                     // sine rule; the - sign corrects for above/below horizontal
  
  return static_cast<float>(α);
}
...
{ for (int delta_y { delta_y_start }; delta_y <= n_cells; delta_y += delta_y_increment)
  { for (int delta_x { -n_cells }; delta_x <= n_cells; ++delta_x)
    { const int                  column_index       { delta_x + n_cells };            // >= 0
      const int                  row_index          { delta_y + n_cells };            // >= 0
      const double               bearing_from_north { bearing(delta_x, delta_y) };
      const double               distance_to_square { sqrt(1.0 * delta_x * delta_x + 1.0 * delta_y * delta_y) * distance_per_square };    // along curved surface
      const pair<double, double> ll                 { ll_from_bd(qth, bearing_from_north, distance_to_square) };
...
// see note near the top of the file regarding modification of the received heights
      ...
      catch (const geotiff_error& e)
      { cerr << "Caught geotiff error while calculating height field: " << e.reason() << endl;
      ...
      { if ( (delta_x == 0) and (delta_y == 0) )
        ...
        { try
          ...
          { cerr << "Caught geotiff error while calculating grad field: " << e.reason() << endl;
...
{ vector<string> labels;
  ...
  FOR_ALL(distances_km, [&labels] (const auto d) { labels += to_string(d); } );
...
{ execute_r(R, "text(x=0.5, y = 0.65, labels = c('" + callsign + "'), cex = 2.3, font = 2, col = 'BLACK', family ='Noto Mono', adj = c(0.5, 0) )");  // basic call
  ...
  if (contains(callsign, '0'))
    execute_r(R, "text(x=0.5, y = 0.65, labels = c('" + slash_overstrike(callsign) + "'), cex = 2.3, font = 2, col = 'BLACK', family ='Noto Mono', adj = c(0.5, 0) )"); // superimpose slashes on zeros

  const string latitude_str  { to_string(latitude) };
  const string lat_str       { substring <string> (latitude_str, 0, 2) + "°·" + substring <string> (latitude_str, 3, 5) + "N" };
  const string longitude_str { to_string(-longitude) };
  const string long_str      { substring <string> (longitude_str, 0, longitude <= -100 ? 3 : 2) + "°·" + substring <string> (longitude_str, longitude <= -100 ? 4 : 3, 5) + "W" };
...
{ const vector<int> vi { r_seq(int(min_horizon), int(max_horizon), (int(max_horizon - min_horizon) / 10) + 1 ) };
  ...
  FOR_ALL(vi, [&vs] (const int i) { vs += to_string(i); });
